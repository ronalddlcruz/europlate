...
export const exchangeRateRepository = {
  list: (companyId: string) => prisma.exchangeRate.findMany({ where: { companyId }, include, orderBy: [{ effectiveDate: 'desc' }, { createdAt: 'desc' }] }),
  current: (companyId: string) => prisma.exchangeRate.findFirst({ where: { companyId }, include, orderBy: [{ effectiveDate: 'desc' }, { createdAt: 'desc' }] }),
  findById: (companyId: string, id: string) => prisma.exchangeRate.findFirst({ where: { id, companyId }, include }),
