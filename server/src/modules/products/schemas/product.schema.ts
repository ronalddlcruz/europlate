...
const attributeSchema = z.object({ id: z.string().cuid().optional(), name: z.string().trim().min(1).max(80), dataType: attributeType.default('TEXT'), suffix: z.string().trim().max(20).optional().nullable(), required: z.boolean().default(false), status: productStatus.default('ACTIVE'), useInSubtotal: z.boolean().default(false) })
const presentationSchema = z.object({ id: z.string().cuid().optional(), code: z.string().trim().min(1).max(40).optional(), name: z.string().trim().min(1).max(160), unitId: z.string().cuid().optional(), unitCode: z.string().trim().min(1).max(12).optional(), attributeValues: z.record(z.string()).default({}), factor: z.coerce.number().positive().default(1), minimumStock: z.coerce.number().nonnegative().default(0), currentStock: z.coerce.number().nonnegative().default(0), status: productStatus.default('ACTIVE') }).refine(value => value.unitId || value.unitCode, { message: 'Selecciona una unidad de inventario.', path: ['unitId'] })

const productPayloadSchema = z.object({ name: z.string().trim().min(1).max(160), categoryId: z.string().cuid().optional().nullable(), subcategoryId: z.string().cuid().optional().nullable(), brandId: z.string().cuid().optional().nullable(), status: productStatus.default('ACTIVE'), roles: z.array(productRole).min(1), variantType: variantType.default('WITH_VARIANTS'), immediateConsumption: z.boolean().default(true), attributes: z.array(attributeSchema).default([]), presentations: z.array(presentationSchema).default([]) })
