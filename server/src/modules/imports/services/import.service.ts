...
async function resolveItems(db: Prisma.TransactionClient, input: ImportInput): Promise<ResolvedItem[]> {
  return Promise.all(input.items.map(async item => {
    if (!isVariableItem(item)) return { productId: item.productId, presentationId: item.presentationId, warehouseId: item.warehouseId, quantity: item.quantity, unitCostUsd: item.unitCostUsd }
    ...
    return { productId: product.id, presentationId: presentation.id, warehouseId: item.warehouseId, quantity: item.quantity, unitCostUsd: item.unitCostUsd }
  }))
}
...
export const importService = {
  ...
  async update(companyId: string, id: string, input: UpdateImportInput) {
    ...
    const inputItems = input.items?.map(item => {
      ...
      return { productId: item.productId, presentationId: item.presentationId, warehouseId: item.warehouseId, quantity: item.quantity, unitCostUsd: item.unitCostUsd }
    })
    const items = inputItems ?? current.items.map(line => ({ productId: line.productId, presentationId: line.presentationId, warehouseId: line.warehouseId, quantity: Number(line.quantity), unitCostUsd: Number(line.unitCostUsd) }))
