...
import { purchaseInputSchema, purchaseQuerySchema, updatePurchaseSchema } from '../schemas/purchase.schema.js'
...
export const purchaseController = {
  ...
  async uploadDocument(request: Request, response: Response) {
    ...
    if (!fileName || !Buffer.isBuffer(request.body) || request.body.length === 0) throw new AppError('PURCHASE_DOCUMENT_INVALID', 'Adjunta un archivo PDF válido.', 422)
    response.status(201).json({ data: await purchaseService.uploadDocument(request.auth!.companyId, decodeURIComponent(fileName), request.body) })
  },
  async removeDocument(request: Request, response: Response) {
    ...
    if (!storageKey) throw new AppError('PURCHASE_DOCUMENT_INVALID', 'No se encontró el archivo adjunto.', 422)
    await purchaseService.removeDocument(request.auth!.companyId, storageKey)
