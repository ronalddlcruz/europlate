...
import type { ImportCatalog } from '../../imports/services/import-api.service'
import type { PurchaseCatalog } from '../../purchases/services/purchase-api.service'
...
const updateCatalogSuppliers = <T extends { suppliers: { id: string; name: string }[] }>(catalog: T | undefined, supplier: Supplier, expectedType: SupplierType): T | undefined => {
  if (!catalog) return catalog
  const suppliers = catalog.suppliers.filter(item => item.id !== supplier.id)
  if (supplier.type === expectedType && supplier.status === 'Activo') suppliers.push({ id: supplier.id, name: supplier.name })
  return { ...catalog, suppliers: suppliers.sort((a, b) => a.name.localeCompare(b.name, 'es')) }
}

export function SuppliersPage() {
  const queryClient = useQueryClient()
  const suppliersQuery = useQuery({ queryKey: ['suppliers'], queryFn: listSuppliers, staleTime: 5 * 60 * 1000, gcTime: 15 * 60 * 1000 })
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (saved) => {
      ...
      queryClient.setQueryData<PurchaseCatalog>(['purchases', 'catalog'], current => updateCatalogSuppliers(current, saved, 'Nacional'))
      queryClient.setQueryData<ImportCatalog>(['imports', 'catalog'], current => updateCatalogSuppliers(current, saved, 'Extranjero'))
      void queryClient.invalidateQueries({ queryKey: ['suppliers'] })
      void queryClient.invalidateQueries({ queryKey: ['purchases', 'catalog'] })
      void queryClient.invalidateQueries({ queryKey: ['imports', 'catalog'] })
