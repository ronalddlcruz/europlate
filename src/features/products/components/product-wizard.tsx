...
export function ProductWizard({ item, variants: initialVariants = [], products, categories, units, onClose, onSave, presetSubcategoryId, saveLabel = 'Guardar producto' }: { item?: ProductBase; variants?: ProductVariant[]; products: ProductBase[]; categories: ProductCategory[]; units: Unit[]; onClose: () => void; onSave: (value: SaveValue) => void | Promise<void>; presetSubcategoryId?: string; saveLabel?: string }) {
  const initialVariant = initialVariants[0]
  const presetCategory = categories.find(category => category.subcategories.some(subcategory => subcategory.id === presetSubcategoryId))
  const presetSubcategory = presetCategory?.subcategories.find(subcategory => subcategory.id === presetSubcategoryId)
  const [categoryId, setCategoryId] = useState(item?.categoryId ?? presetCategory?.id ?? '')
  const [subcategoryId, setSubcategoryId] = useState(item?.subcategoryId ?? presetSubcategoryId ?? '')
  ...
  // En un producto variable los valores deben conservar los IDs configurados
  // en categoría/subcategoría. El backend los valida antes de crear el producto.
  const [attributes, setAttributes] = useState<Attribute[]>(item?.attributes ?? (presetCategory && presetSubcategory ? [...presetCategory.attributes, ...presetSubcategory.attributes] : []))
  ...
  const subcategories = category?.subcategories.filter(candidate => candidate.status === 'Activo' && (!candidate.isVariable || candidate.id === presetSubcategoryId)) ?? []
  ...
  return <Dialog open wide title={presetSubcategoryId ? 'Completar producto variable' : item ? 'Editar producto' : 'Nuevo producto'} onClose={onClose} footer={<><Button variant="outline" onClick={onClose}>Cancelar</Button><Button onClick={() => void submit()}>{saveLabel}</Button></>}>
