...
function productPayload(base: ProductBase, variants: ProductVariant[], units: Unit[]) {
  return {
    name: base.name, categoryId: base.categoryId || null,
...
export async function createCatalogCategory(name: string) { return api<ProductCategory>('/api/products/categories', { method: 'POST', body: JSON.stringify({ name }) }) }
export async function updateCatalogCategory(id: string, name: string) { return api<ProductCategory>(`/api/products/categories/${id}`, { method: 'PATCH', body: JSON.stringify({ name }) }) }
export async function deleteCatalogCategory(id: string) { await api<void>(`/api/products/categories/${id}`, { method: 'DELETE' }) }
