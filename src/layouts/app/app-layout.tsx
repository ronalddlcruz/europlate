...
function NavItem({ label, icon: Icon, to, onNavigate }: NavigationItem & { onNavigate?: () => void }) {
  return to ? <NavLink to={to} onClick={onNavigate} className={({ isActive }) => cn('flex items-center gap-2.5 border-l-3 border-transparent px-5 py-2 text-[13.5px] text-slate-600 transition hover:bg-[#eff4ff] hover:text-brand', isActive && 'border-brand bg-[#eff4ff] font-medium text-brand')}><Icon className="h-[17px] w-[17px]" />{label}</NavLink> : <button type="button" onClick={onNavigate} className="flex w-full items-center gap-2.5 border-l-3 border-transparent px-5 py-2 text-left text-[13.5px] text-slate-600 transition hover:bg-[#eff4ff] hover:text-brand"><Icon className="h-[17px] w-[17px]" />{label}</button>
}
...
function Sidebar({ onNavigate, onClose }: { onNavigate?: () => void; onClose?: () => void }) {
  ...
  return <>
    <div className="flex items-center justify-between border-b border-border px-5 py-4"><div><p className="font-mono text-[17px] font-bold tracking-tight text-brand">EUROPLATE</p><p className="mt-0.5 text-[11px] text-muted">Gestión Comercial</p></div>{onClose && <button type="button" onClick={onClose} className="rounded-md p-2 text-slate-500 hover:bg-slate-100 lg:hidden" aria-label="Cerrar menú"><X className="h-5 w-5" /></button>}</div>
...
export function AppLayout() {
  ...
  return <div className="min-h-screen bg-page text-ink lg:pl-[220px]">
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-[220px] flex-col border-r border-border bg-white lg:flex"><Sidebar /></aside>
