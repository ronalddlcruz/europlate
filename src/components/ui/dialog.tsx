...
export function Dialog({ open, title, children, onClose, footer, wide, extraWide }: DialogProps) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/35 p-0 sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-label={title}>
    <section className={`max-h-[94dvh] w-full overflow-auto rounded-t-[14px] border border-border bg-white shadow-panel sm:max-h-[92vh] sm:rounded-[10px] ${extraWide ? 'max-w-[1280px]' : wide ? 'max-w-[1080px]' : 'max-w-[680px]'}`}>
